...
TEXTS: dict[str, dict[str, str]] = {
    "zh": {
        ...
        "cli.stop.pid_mismatch": "pid 文件指向 {pid}，但 socket 上的 daemon 不是它（{other}），不发信号；请手动核实",
        "cli.stop.no_response": "无回应",
        "cli.stop.signal_failed": "向 pid {pid} 发 SIGTERM 失败：{error}",
        ...
        "daemon_error.socket": "起不了 socket {path}：{error}。unix socket 路径有长度上限（macOS 104 字节），换一个短一点的 AGENT_NTFY_HOME",
    ...
    "en": {
        ...
        "cli.stop.pid_mismatch": "the pid file points at {pid}, but the daemon on the socket is not it ({other}); not signalling — please check by hand",
        "cli.stop.no_response": "no response",
        "cli.stop.signal_failed": "sending SIGTERM to pid {pid} failed: {error}",
        ...
        "daemon_error.socket": "can't bind the socket {path}: {error}. Unix socket paths have a length limit (104 bytes on macOS); pick a shorter AGENT_NTFY_HOME",
