...
BODY_FIELD = "body"  # 「正文」那条问题挂的字段名；显示时按语言取（validate.field.body）
...
def check(payload: object, lang: str) -> list[Problem]:
    ...
    if payload.get("lang") is not None and not texts.is_lang(payload["lang"]):  # null 当没给
        ...
        problems.append(Problem("lang", texts.t("validate.lang", lang, value=payload["lang"], choices=" / ".join(texts.LANGS))))
...
def _field_order(p: Problem) -> int:
    """按契约里的字段顺序排（同字段保持产生顺序），lang 与正文最后——与 agent 手里那份 JSON 的顺序一致，改起来顺手。"""
    names = list(REQUIRED_FIELDS) + ["lang"]
...
def check_json(text: str, fallback_lang: str = texts.DEFAULT_LANG) -> tuple[dict | None, list[Problem], str]:
    ...
    problems = check(payload, lang)
    return (payload if isinstance(payload, dict) and not problems else None), problems, lang


def field_label(field: str, lang: str) -> str:
    """问题挂的字段名的显示形态：只有「正文」这条是按语言取的，其余就是 JSON 字段名。"""
    return texts.t("validate.field.body", lang) if field == BODY_FIELD else field
...
def format_problems(problems: list[Problem], lang: str) -> str:
    """排成 stderr 文案。首行点明「消息未发送」，之后一行一个问题，字段名对齐。"""
    lines = [texts.t("validate.header", lang, n=len(problems)), ""]
    lines += [f"  {_pad(field_label(p.field, lang), FIELD_WIDTH)}: {p.message}" for p in problems]
    return "\n".join(lines) + "\n"
