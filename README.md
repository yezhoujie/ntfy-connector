...
| The daemon and every other subcommand: `confirm-sub`, `slots`, `release`, `add-slot`, `away` | `confirm-sub` opening a pane for you (without herdr it falls back to asking you to run `confirm-sub` yourself) |
...
```bash
python3 "<skill dir>/scripts/ntfy_connector.py" daemon --detach        # any platform, in or out of herdr:  daemon: started in the background, pid 12345 (log ~/.ntfy-connector/daemon.log)
...
- ntfy.sh's free tier allows about **250 messages per day per source IP**, shared by questions, notifications, "answered" updates, receipts and confirmations. Enough for normal use; if not, self-host ntfy and set `NTFY_CONNECTOR_URL`.
